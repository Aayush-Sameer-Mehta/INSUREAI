...
export default function ResetPassword() {
  ...
  // Auto-fill token from URL query params on mount
  useEffect(() => {
  ...
  return (
    <div className="flex min-h-[75vh] items-center justify-center px-4">
      ...
      <div className="pointer-events-none fixed inset-0 overflow-hidden">
        <div className="absolute -left-32 -top-32 h-96 w-96 rounded-full bg-gradient-to-br from-rose-400/20 to-pink-400/20 blur-3xl" />
        <div className="absolute -bottom-32 -right-32 h-96 w-96 rounded-full bg-gradient-to-br from-violet-400/15 to-indigo-400/15 blur-3xl" />
      </div>

      <div className="relative z-10 w-full max-w-md">
        {/* ─── Icon header ─────────────────────────────── */}
        <div className="mb-8 text-center">
          <div className="mx-auto mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-gradient-to-br from-rose-500 to-pink-600 shadow-xl shadow-rose-500/25">
            <Shield className="h-8 w-8 text-white" />
          </div>
          <h1 className="text-3xl font-bold tracking-tight text-slate-900 ">
            Reset Password
          </h1>
          <p className="mt-2 text-slate-500 ">Enter your new password</p>
        </div>
            ...
            {FIELDS.map(({ key, label, type, placeholder, icon: Icon }) => (
              <div key={key}>
                ...
                >
                  <input
                    type={
                      type === "password" && showPasswords[key] ? "text" : type
                    }
